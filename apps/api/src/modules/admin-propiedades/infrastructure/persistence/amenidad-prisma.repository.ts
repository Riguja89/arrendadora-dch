...
/** Adaptador Prisma del catálogo `amenidades` (ADR-005). */
@Injectable()
export class AmenidadPrismaRepository implements CatalogoRepositoryPort {
  constructor(private readonly prisma: PrismaService) {}

  async listar(incluirInactivos: boolean): Promise<CatalogoItem[]> {
    const registros = await this.prisma.amenidad.findMany({
  ...
  async buscarPorId(id: string): Promise<CatalogoItem | null> {
    const registro = await this.prisma.amenidad.findUnique({ where: { id } });
    return registro ? aCatalogoDominio(registro) : null;
  }

  async buscarPorNombre(nombre: string): Promise<CatalogoItem | null> {
    const registro = await this.prisma.amenidad.findFirst({
  ...
  async guardar(item: CatalogoItem): Promise<void> {
    ...
    await this.prisma.amenidad.upsert({ where: { id: data.id }, create: data, update: data });
  }
  ...
  async existenActivos(ids: string[]): Promise<boolean> {
    ...
    const total = await this.prisma.amenidad.count({
